...
This is the member that exists because the other two do. `sony-spc700-python`
models the processor and `sony-s-dsp-python` models the sound generator, and
neither models the other, because two models of one wire is how two models
disagree. What is left once you have both is the thing between them: sixty four
kilobytes they share, four ports the console reaches, three timers, a control
register that changes what the address space is, and sixty four bytes of boot
program in the top page.

It is a board rather than a part or a clocked part. It has no instruction set of
its own, and the cycles it counts are the ones its processor spends, charged
through so the timers advance at their own rate rather than once per instruction.

**It needs a boot program and carries none.** Those sixty four bytes are Sony's.
Without a copy the unit refuses to be built and says why, and every check that
needs one skips out loud rather than reporting a pass.

## What the boot program settled

Composing the two halves surfaced a defect in one of them, which is the reason
this member is worth having beyond tidiness.
...
gets its audio program uploaded. Every Super Nintendo that has made a sound is
the measurement, and the fix is in `sony-spc700-python` rather than here.

## What is checked, and against what

The upload protocol, end to end, played at the unit the way a cartridge plays it:
the handshake, the destination, the block, the acknowledgement of every byte, a
second block after the first, and the jump. Nothing in it was copied from an
implementation of either half. It is read off the boot program, and the boot
program is Sony's.

That puts the evidence for this member at the second rung of the ladder above
rather than the fourth. What it does not reach is timing: the console and the
unit are clocked separately and neither divides the other, so no figure here
converts one to the other and none is published.
